...
export default function TenantContractsPage() {
  ...
  const handleEdit = (contract: TenantContract) => {
    setEditingContract(contract);
    setIsModalOpen(true);
  };

  const handleDelete = async (contract: TenantContract) => {
    if (confirm(`確定要刪除「${contract.tenant_name}」的合約嗎？`)) {
      await fetch(`/api/tenant-contracts?id=${contract.id}`, { method: 'DELETE' });
      fetchContracts();
    }
  };
  ...
  const columns = [
    ...
    { 
      key: 'end_date', 
      label: '到期日',
      render: (v: string, row: TenantContract) => (
        <div>
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-800">👤 房客合約</h1>
        <button
